...
/**
 * Monkey-patch a viewport instance to log stack-changing calls.
 *
 * - Logs via console.trace so you get full call stack in devtools.
 * - Pushes a lightweight entry into window.__viewerLog for persistence.
 * - Protects against double-patching.
 *
 * Usage: call once after you obtain vp (e.g. right after engine.getViewport(VIEWPORT_ID)):
 *   import { instrumentViewportLogging } from '@/lib/cornerstoneHelpers';
 *   instrumentViewportLogging(vp);
 */
export function instrumentViewportLogging(vp: any): void {
  ...
  try {
    ...
    if ((vp as any).__cs_instrumented_for_logging) return;
    (vp as any).__cs_instrumented_for_logging = true;
  } catch {
    // continue even if we can't set the flag
  }

  try {
    const wrapAndTrace = (name: string, origFn: any) => {
      if (typeof origFn !== 'function') return origFn;

      const wrapped = function(this: any, ...args: any[]) {
        try {
          // print a full trace in console (stack included)
          try {
            console.trace(`🔥 vp.${name} called`, args);
          } catch {}

          // also push a lightweight record into window.__viewerLog for persistence
          try {
            const w: any = window;
            w.__viewerLog = w.__viewerLog || [];
            const argSummary = (args && args.length > 0) ? safeInspect(args[0], 6) : null;
            w.__viewerLog.push({
              t: Date.now(),
              evt: `vp.${name}.called`,
              argSummary,
            });
            if (w.__viewerLog.length > 2000) w.__viewerLog.shift();
          } catch {}
        } catch {
        ...
        try {
          ...
          try { console.error(`vp.${name} threw`, err); } catch {}
          throw err;
        }
      };

      try {
        Object.defineProperty(wrapped, 'name', { value: `instrumented_${name}`, configurable: true });
      } catch {}
    ...
    try {
      ...
      if (typeof origSetStack === 'function') {
        (vp as any).setStack = wrapAndTrace('setStack', origSetStack);
      }
    } catch {}
    ...
    try {
      ...
      if (typeof origSetImageId === 'function') {
        (vp as any).setImageId = wrapAndTrace('setImageId', origSetImageId);
      }
    } catch {}

    try {
      const origSetImageIndex = (vp as any).setImageIndex;
      if (typeof origSetImageIndex === 'function') {
        (vp as any).setImageIndex = wrapAndTrace('setImageIndex', origSetImageIndex);
      }
    } catch {}
  } catch (e) {
    try { console.debug('instrumentViewportLogging failed', e); } catch {}
  }
}
